...
export function UpdateDeck({ handleClose, selectedItem }) {
    ...
    useEffect(() => {
        if (selectedItem) {
            setDeckName(selectedItem.deck_name);
            setSelectedTag(selectedItem.tag.id);
        }
    }, [selectedItem]);
    ...
    async function handleCreateDeck(){
        ...
        if (!deckName) {
            ...
            console.error('Deck must have a tag! '); 
                ...
                try {
                    await EditDeck(selectedItem.id, data, ip, token).then(() => {
                        handleClose();
                    }).catch(() => {
                        Toast.show({
                            type: 'error',
    ...
    return (
        <SafeAreaView style={styles.container}>
            <View style={styles.cardHeader}>
                <Text style={styles.textButton}>Edit a deck</Text>
            </View>
